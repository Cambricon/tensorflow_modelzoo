...
class Synthesizer:
	def load(self, checkpoint_path, hparams, gta=False, model_name='Tacotron'):
		...
		split_infos = tf.placeholder(tf.int32, shape=(hparams.tacotron_num_devices, None), name='split_infos')
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames):
		...
		assert 0 == len(texts) % self._hparams.tacotron_num_devices
		seqs = [np.asarray(text_to_sequence(text, cleaner_names)) for text in texts]
		input_lengths = [len(seq) for seq in seqs]

		size_per_device = len(seqs) // self._hparams.tacotron_num_devices

		#Pad inputs according to each GPU max length
		input_seqs = None
		split_infos = []
		for i in range(self._hparams.tacotron_num_devices):
		...
		if self.gta or not hparams.predict_linear:
			...
			mels = [mel for device_mels in mels for mel in device_mels]
			alignments = [align for device_aligns in alignments for align in device_aligns]
			stop_tokens = [token for device_token in stop_tokens for token in device_token]
			...
			#Linearize outputs (1D arrays)
			linears = [linear for device_linear in linears for linear in device_linear]
			mels = [mel for device_mels in mels for mel in device_mels]
			alignments = [align for device_aligns in alignments for align in device_aligns]
			stop_tokens = [token for device_token in stop_tokens for token in device_token]
