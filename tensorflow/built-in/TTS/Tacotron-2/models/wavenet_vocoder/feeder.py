...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		with tf.device('/cpu:0'):
			...
			tf.placeholder(tf.int32, shape=(hparams.tacotron_num_devices, None), name='split_infos'),
			]
	...
	def _prepare_batch(self, batches, outputs_per_step):
		assert 0 == len(batches) % self._hparams.tacotron_num_devices
		size_per_device = int(len(batches) / self._hparams.tacotron_num_devices)
		np.random.shuffle(batches)
