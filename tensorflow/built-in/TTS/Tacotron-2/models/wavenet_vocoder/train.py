...
def add_train_stats(model, hparams):
	with tf.variable_scope('stats') as scope:
		for i in range(hparams.tacotron_num_devices):
...
def train(log_dir, args, hparams):
	...
	if args.use_performance and (not args.use_horovod or (args.use_horovod and hvd.rank() == 0)):
		times = []

	checkpoint_path = os.path.join(save_dir, 'tacotron_model.ckpt')
	if args.taco_checkpoint:
		load_dir = args.taco_checkpoint
	else:
		load_dir = save_dir
	...
	batch_size = args.tacotron_batch_size
	config = tf.ConfigProto()
	config.allow_soft_placement = True
	if args.use_horovod:
		if args.device_type == 'gpu':
			config.gpu_options.visible_device_list = str(hvd.local_rank())
		elif args.device_type == 'mlu':
	...
	with tf.Session(config=config) as sess:
		try:
			...
			if args.restore:
				...
				try:
					...
					if (checkpoint_state and checkpoint_state.model_checkpoint_path):
						...
						log('No model to load at {}'.format(load_dir), slack=True)
			...
			while not coord.should_stop() and step < args.tacotron_train_steps:
				...
				elif step % args.embedding_interval == 0 or step == args.tacotron_train_steps:
					#Get current checkpoint state
			...
			if args.use_profiler:
				if not args.use_horovod or (args.use_horovod and hvd.rank() == 0):
			...
			if args.use_performance:
				if not args.use_horovod:
					write_json("summary", batch_size, times)
				elif (args.use_horovod and hvd.rank() == 0):
					write_json("summary", batch_size*hvd.size(), times)
