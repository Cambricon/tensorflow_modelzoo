...
class Tacotron():
	...
	def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None, targets_lengths=None, gta=False,
		...
		split_device = '/cpu:0' if self._hparams.tacotron_num_devices > 1 or self._hparams.split_on_cpu else '/{}:{}'.format(self._hparams.device_type, self._hparams.tacotron_device_start_idx)
		with tf.device(split_device):
			hp = self._hparams
			lout_int = [tf.int32]*hp.tacotron_num_devices
			lout_float = [tf.float32]*hp.tacotron_num_devices

			tower_input_lengths = tf.split(input_lengths, num_or_size_splits=hp.tacotron_num_devices, axis=0)
			tower_targets_lengths = tf.split(targets_lengths, num_or_size_splits=hp.tacotron_num_devices, axis=0) if targets_lengths is not None else targets_lengths
			...
			for i in range (hp.tacotron_num_devices):
		...
		# 1. Declare Devices
		devices = ["/{}:{}".format(hp.device_type, i) for i in range(hp.tacotron_device_start_idx, hp.tacotron_device_start_idx+hp.tacotron_num_devices)]
		for i in range(hp.tacotron_num_devices):
			with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device=devices[i])):
			...
			log('initialisation done {}'.format(devices[i]))
		...
		for i in range(hp.tacotron_num_devices+hp.tacotron_device_start_idx):
			log('  device:                   {}'.format(i))
	...
	def add_loss(self):
		...
		self.before_loss = total_before_loss / hp.tacotron_num_devices
		self.after_loss = total_after_loss / hp.tacotron_num_devices
		self.stop_token_loss = total_stop_token_loss / hp.tacotron_num_devices
		self.regularization_loss = total_regularization_loss / hp.tacotron_num_devices
		self.linear_loss = total_linear_loss / hp.tacotron_num_devices
		self.loss = total_loss / hp.tacotron_num_devices
	...
	def add_optimizer(self, global_step):
		...
		grad_device = '/cpu:0' if hp.tacotron_num_devices > 1 else devices[0]
