...
hparams = tf.contrib.training.HParams(
	...
	#If you only have 1 DEVICE or want to use only one DEVICE, please set num_devices=0 and specify the DEVICE idx on run. example:
		#expample 1 DEVICE of index 2 (train on "/device2" only): DEVICE(MLU OR GPU)_VISIBLE_DEVICES=2 python train.py --model='Tacotron' --hparams='tacotron_device_start_idx=2'
	#If you want to train on multiple DEVICEs, simply specify the number of DEVICEs available, and the idx of the first DEVICE to use. example:
		#example 4 DEVICEs starting from index 0 (train on "/device0"->"/device3"): python train.py --model='Tacotron' --hparams='tacotron_num_devices=4, tacotron_device_start_idx=0'
	#The hparams arguments can be directly modified on this hparams.py file instead of being specified on run if preferred!

	#If one wants to train both Tacotron and WaveNet in parallel (provided WaveNet will be trained on True mel spectrograms), one needs to specify different DEVICE idxes.
	#example Tacotron+WaveNet on a machine with 4 or plus DEVICEs. Two DEVICEs for each model:
		# CUDA_VISIBLE_DEVICES=0,1 python train.py --model='Tacotron' --hparams='tacotron_device_start_idx=0, tacotron_num_devices=2'
		# Cuda_VISIBLE_DEVICES=2,3 python train.py --model='WaveNet' --hparams='wavenet_device_start_idx=2; wavenet_num_devices=2'

	#IMPORTANT NOTE: If using N DEVICEs, please multiply the tacotron_batch_size by N below in the hparams! (tacotron_batch_size = 32 * N)
	#Never use lower batch size than 32 on a single DEVICE!
	#Same applies for Wavenet: wavenet_batch_size = 8 * N (wavenet_batch_size can be smaller than 8 if DEVICE is having OOM, minimum 2)
	#Please also apply the synthesis batch size modification likewise. (if N DEVICEs are used for synthesis, minimal batch size must be N, minimum of 1 sample per DEVICE)
	#We did not add an automatic multiMLPU batch size computation to avoid confusion in the user's mind and to provide more control to the user for
	#resources related decisions.

	#Acknowledgement:
	#	Many thanks to @MlWoo for his awesome work on multi-DEVICE Tacotron which showed to work a little faster than the original
	#	pipeline for a single DEVICE as well. Great work!

	#Hardware setup: Default supposes user has only one DEVICE: "/device:0" (Tacotron only for now! WaveNet does not support multi DEVICE yet, WIP)
	#Synthesis also uses the following hardware parameters for multi-DEVICE parallel synthesis.
	device_type='mlu', #which device to use, must be mlu/gpu/cpu.
	use_amp = False, #use mixed precision or not.
	use_horovod = False, #use horovod or not.
	use_profiler = False, #use profiler or not.
	use_performance = False, #use performance or not.
	tacotron_device_start_idx = 0, #idx of the first DEVICE to be used for Tacotron training.
	tacotron_num_devices = 1, #Determines the number of devices in use for Tacotron training.
	wavenet_device_start_idx = 0, #idx of the first DEVICE to be used for WaveNet training. (WIP)
	wavenet_num_devices = 1, #Determines the number of devices in use for WaveNet training. (WIP)
	split_on_cpu = True, #Determines whether to split data on CPU or on first DEVICE. This is automatically True when more than 1 DEVICE is used.
	###########################################################################################################################################
	...
	max_time_steps = 11000, #Max time steps in audio used to train wavenet (decrease to save memory) (Recommend: 8000 on modest DEVICEs, 13000 on stronger ones)
	###########################################################################################################################################
	...
	tacotron_swap_with_cpu = False, #Whether to use cpu as support to device for decoder computation (Not recommended: may cause major slowdowns! Only use when critical!)
	...
	#performance parameters
	wavenet_swap_with_cpu = False, #Whether to use cpu as support to device for synthesis computation (while loop).(Not recommended: may cause major slowdowns! Only use when critical!)
	...
	#Usually your DEVICE can handle ~2x wavenet_batch_size during synthesis for the same memory amount during training (because no gradients to keep and ops to register for backprop)
