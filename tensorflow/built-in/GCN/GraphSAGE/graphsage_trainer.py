...
from models.modeling.models import SampleAndAggregate, SAGEInfo, Node2VecModel
from models.modeling.minibatch import EdgeMinibatchIterator
from models.modeling.neigh_samplers import UniformNeighborSampler
from models.modeling.utils import load_data
...
flags.DEFINE_string('data_dir', '', 'name of the object file that stores the training data. must be specified.')
...
flags.DEFINE_boolean('use_amp', False, 'whether to use auto mix precision')
...
# tf-perf settings.
flags.DEFINE_bool("use_profiler", False, "use profiler to train nets or not")
flags.DEFINE_bool("use_performance", False, "use performance tools to get fps or not")


def log_dir():
    log_dir = FLAGS.base_log_dir + "/unsup-" + FLAGS.data_dir.split("/")[-2]
    ...
    for epoch in range(FLAGS.epochs):
        ...
        while not minibatch.end():
            ...
            outs = sess.run([merged, model.opt_op, model.loss, model.ranks, model.aff_all,
                    model.mrr, model.outputs1], feed_dict=feed_dict, options=run_options, run_metadata=run_metadata)
            if FLAGS.use_performance:
...
def main(argv=None):
    ...
    train_data = load_data(FLAGS.data_dir, load_walks=True)
