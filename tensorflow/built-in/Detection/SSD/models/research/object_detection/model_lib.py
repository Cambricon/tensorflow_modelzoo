...
from models.research.object_detection import eval_util
from models.research.object_detection import exporter as exporter_lib
from models.research.object_detection import inputs
...
def create_model_fn(detection_model_fn, configs, hparams, use_tpu=False, use_horovod=False, use_amp=False):
...
#                train_config.load_all_detection_checkpoint_vars))
    ...
    if mode in (tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL):
      ...
      training_optimizer, optimizer_summary_vars = optimizer_builder.build(
          train_config.optimizer, use_horovod=use_horovod)
...
def create_estimator_and_inputs(run_config,
  ...
  Returns:
    ...
    'train_steps': Number of training steps. Either directly from input or from
      configuration.
    'train_batch_size': train batch size per MLU
  """
  ...
  if(kwargs['fine_tune_checkpoint']):
      configs['train_config'].fine_tune_checkpoint = kwargs['fine_tune_checkpoint']
  if(kwargs['fine_tune_checkpoint_type']):
      configs['train_config'].fine_tune_checkpoint_type = kwargs['fine_tune_checkpoint_type']
  ...
  model_fn = model_fn_creator(detection_model_fn, configs, hparams, use_tpu, use_horovod, use_amp)
