...
class RpnScoreLoss(object):
  ...
  def __call__(self, score_outputs, labels):
    ...
    Args:
      ...
      labels: the dictionary that returned from models.dataloader that includes
        groundturth targets.
...
class RetinanetBoxLoss(object):
  ...
  def __init__(self, params):
    self._huber_loss = tf.keras.losses.Huber(
        delta=params.huber_loss_delta, reduction=tf.compat.v2.keras.losses.Reduction.SUM)
