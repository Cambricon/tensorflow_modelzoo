...
import models.modeling.modeling as modeling
import models.modeling.optimization as optimization
import models.modeling.tokenization as tokenization
from models.utils.create_squad_data import *
from models.utils.utils import LogEvalRunHook, LogTrainRunHook
import models.utils.dllogger_class
...
def extract_run_squad_flags():
  ...
  flags.DEFINE_string(
      "hvd_device", None,
      "Horovod`s equipment used for training.")

  flags.DEFINE_integer(
      "train_steps", 0,
      "The steps of mixed precision training.")
  ...
  flags.DEFINE_bool("use_horovod", False, "Whether to use Horovod for multi-mlu runs")
  flags.DEFINE_bool("use_profiler", False, "Whether to use profiler")
  flags.DEFINE_bool("use_performance", False, "Whether to use use_performance test tools")
  flags.DEFINE_integer("inter_op_threads", 0, "inter op threads.")
  flags.DEFINE_integer("intra_op_threads", 0, "intra op threads.")
  ...
  flags.DEFINE_bool("amp", False, "Whether to enable AMP ops. When false, uses TF32 on MLUS.")
...
def main(_):
  ...
  dllogging = models.utils.dllogger_class.dllogger_class(FLAGS.dllog_path)
  ...
  if FLAGS.use_horovod:
      ...
      tf.compat.v1.logging.info("Multi-MLU training with TF Horovod")
  ...
  training_hooks.append(LogTrainRunHook(
      global_batch_size, hvd_rank, FLAGS.save_checkpoints_steps, 0))
  ...
  model_fn = model_fn_builder(
      ...
      hvd=None if not FLAGS.use_horovod else hvd,
      amp=FLAGS.amp)
