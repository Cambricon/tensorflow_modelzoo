...
flags.DEFINE_string("middle_output", "models/middle_data", "Dir was used to store middle data!")
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder, hvd=None):
    name_to_features = {
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            train_op = optimization.create_optimizer(total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, FLAGS.use_horovod, FLAGS.use_amp)
...
def main(_):
    ...
    if FLAGS.use_horovod:
        ...
        global hvd
        hvd.init()
        FLAGS.output_dir = FLAGS.output_dir + '/' + str(hvd.rank())
    ...
    run_config = tf.contrib.tpu.RunConfig(
        ...
        save_checkpoints_steps=FLAGS.save_checkpoints_steps if (not FLAGS.use_horovod) or (FLAGS.use_horovod and hvd.rank() == 0) else None,
    ...
    if FLAGS.finetune_steps != 0:
        num_train_steps = FLAGS.finetune_steps
    ...
    if FLAGS.do_train:
        ...
        if FLAGS.use_horovod:
            ...
            _,_ = filed_based_convert_examples_to_features(
                train_examples[start_index:end_index], label_list, FLAGS.max_seq_length, tokenizer, train_file[hvd.rank()])
        else:
          _,_ = filed_based_convert_examples_to_features(
        ...
        train_input_fn = file_based_input_fn_builder(
            input_file=train_file if not FLAGS.use_horovod else train_file[hvd.rank()],
        ...
        if FLAGS.use_performance:
            if not FLAGS.use_horovod:
                write_json("summary", FLAGS.train_batch_size, time_hooks.times)
            elif FLAGS.use_horovod and hvd.rank() == 0:
                write_json("summary", FLAGS.train_batch_size * hvd.size(), time_hooks.times)
    ...
    if (not FLAGS.use_horovod) or (FLAGS.use_horovod and hvd.rank() == 0):
        if FLAGS.do_predict:
            ...
            cmd = "perl conlleval.pl -d '\t' < " + output_predict_file
            os.system(cmd)
