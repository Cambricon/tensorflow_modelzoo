...
from nlp.transformer import model_params
...
def define_transformer_flags():
  ...
  flags.DEFINE_integer(
    name="num_mlus", short_name="nm",
    default=1,
    help=flags_core.help_wrap(
        "How many MLUs to use at each worker with the "
        "DistributionStrategies API. The default is 1."))
  flags.DEFINE_integer(
    name="tensorboard_steps", short_name="tbs",
    default=10,
    help=flags_core.help_wrap(
        "How steps to save Tensorboard. The default is 1."))
  flags.DEFINE_integer(
    name="save_ckpt_steps", short_name="scs",
    default=1000,
    help=flags_core.help_wrap(
        "How steps to save checkpoint. The default is 1."))
  flags.DEFINE_string(
    name="finetune_dir",
    default="./float_1gpu",
    help="finetune_dir")
...
def get_callbacks(steps_per_epoch):
  ...
  print_loss_hook = tf.keras.callbacks.LambdaCallback(
          ...
          on_batch_end=lambda batch, logs: print(" real_loss: %.6f" % (logs['loss']), end=" "))
  ...
  if FLAGS.enable_tensorboard:
    tensorboard_callback = tf.keras.callbacks.TensorBoard(
        log_dir=FLAGS.model_dir, batch_size=FLAGS.batch_size,
        update_freq=FLAGS.tensorboard_steps,
        profile_batch=0)
...
class TimeHistory(tf.keras.callbacks.Callback):
  ...
  def on_batch_end(self, batch, logs=None):
    ...
    print("time: {:.6f} s".format(self.end_time-self.start_time))
