...
from models.official.transformer import translate
from models.official.transformer.model import model_params
from models.official.transformer.model import transformer
from models.official.transformer.utils import dataset
from models.official.transformer.utils import metrics
from models.official.transformer.utils import schedule
from models.official.transformer.utils import tokenizer

from models.official.utils.accelerator import tpu as tpu_util
from models.official.utils.export import export
from models.official.utils.flags import core as flags_core
from models.official.utils.logs import hooks_helper
...
def get_train_op_and_metrics(loss, params):
  ...
  with tf.variable_scope("get_train_op"):
    ...
    if params["use_cnmix"] and params["use_amp"]:
      raise ValueError("You can only set use_cnmix in MLU200 or use_amp in MLU300!")

    if params["use_cnmix"]:
      optimizer = cnmix.enable_mixed_precision_graph_rewrite(optimizer)
    ...
    gradients = optimizer.compute_gradients(
        loss, tvars, colocate_gradients_with_ops=False if params["use_mlu"] else True)
...
def run_loop(
    ...
    bleu_source=None, bleu_ref=None, bleu_threshold=None, vocab_file=None, params=None):
  ...
  **Step vs. Epoch vs. Iteration**
  ...
  Args:
    ...
    params: External incoming parameters.
...
def define_transformer_flags():
  ...
  flags.DEFINE_string(
      ...
      help=flags_core.help_wrap("Train or Eval"))
  flags.DEFINE_enum(
  ...
  flags.DEFINE_string(
      ...
      help=flags_core.help_wrap(
          ...
          "the vocab file."))
  flags.DEFINE_bool(
      name="use_profiler", default=False,
      help=flags_core.help_wrap(
  ...
  flags.DEFINE_bool(
      name="use_performance", default=False,
      help=flags_core.help_wrap(
          "Whether use performance test tools."))
  flags.DEFINE_bool(
      name="use_amp", default=False,
      help=flags_core.help_wrap(
          "If use amp, please set True."))
  flags.DEFINE_integer(
      name="inter_op_threads", default=0,
      help=flags_core.help_wrap(
          "inter op threads"))
  flags.DEFINE_integer(
      name="intra_op_threads", default=0,
      help=flags_core.help_wrap(
          "intra op threads"))

  flags.DEFINE_integer(
      name="dropout_seed", default=None,
      help=flags_core.help_wrap(
          "Fix dropout seed for test."))
  flags.DEFINE_integer(
      name="tf_random_seed", default=None,
      help=flags_core.help_wrap(
          "Fix tensorflow random seed for test."))
  flags.DEFINE_bool(
      name="train_data_shuffle", default=True,
      help=flags_core.help_wrap(
          "Switch to control whether shuffle training data or not."))
  flags.DEFINE_bool(
      name="use_mlu", default=True,
      help=flags_core.help_wrap(
          "Switch to control whether use MLU or not."))
  flags.DEFINE_integer(
      name="num_mlus", default=1,
      help=flags_core.help_wrap(
          "the number of MLUs."))
  flags.DEFINE_bool(
      name="use_cnmix", default=False,
      help=flags_core.help_wrap(
          "If use cnmix, please set True."))
  flags.DEFINE_string(
      name="opt_level", short_name="opt",
      default="O0",
      help="The cnmix optimization level of mixed precision training.")
  flags.DEFINE_integer(
      name="data_num",
      default=0,
      help="Number of data sets required for network training.")



  flags_core.set_defaults(data_dir="./translate_ende",
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  if not params["use_tpu"]:
    if params["use_mlu"]:
      distribution_strategy = distribution_utils.get_mlu_distribution_strategy(
        'one_device', params["num_mlus"], None)

      data_num = flags.FLAGS.data_num

      if params["use_cnmix"]:
        session_config = cnmix.mixed_precision_graph_rewrite_config(
                schedule_manager.batch_size,
                data_num,
                opt_level=flags.FLAGS.opt_level)
      else:
        session_config = tf.ConfigProto()

      session_config.inter_op_parallelism_threads=flags_obj.inter_op_threads
      session_config.intra_op_parallelism_threads=flags_obj.intra_op_threads
    else:
      distribution_strategy = distribution_utils.get_distribution_strategy(params["num_gpus"], flags_obj.all_reduce_alg)
      session_config = tf.ConfigProto()
    ...
    return tf.estimator.Estimator(
        ...
        config=tf.estimator.RunConfig(
            ...
            log_step_count_steps=1, save_checkpoints_steps=10000,
            save_summary_steps=10, tf_random_seed=flags_obj.tf_random_seed))
...
def run_transformer(flags_obj):
  ...
  schedule_manager = schedule.Manager(
      ...
      batch_size=params["batch_size"] if not params["use_horovod"] else params["batch_size"] * hvd.size(),
  ...
  if params["mode"]=="eval":
    uncased_score, cased_score = evaluate_and_log_bleu(
        estimator, bleu_source=flags_obj.bleu_source, bleu_ref=flags_obj.bleu_ref, vocab_file=flags_obj.vocab_file)
    return
  run_loop(
      estimator=estimator,
      # Training arguments
