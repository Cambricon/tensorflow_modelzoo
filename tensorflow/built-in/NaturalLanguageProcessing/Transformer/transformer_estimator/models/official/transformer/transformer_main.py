...
def model_fn(features, labels, mode, params):
  ...
  with tf.variable_scope("model"):
    ...
    # /MLU/ freeze the graph for further optimizer
    tf.logging.info("**** Freeze Graph Start ****")
    tf_config = tf.ConfigProto()
    logits_keys = []
    logits_values = []
    for key, value in logits.items():
      logits_keys.append(key)
      logits_values.append(value)

    output_node_names = [i.name.split(":")[0] for i in logits_values]
    with tf.Session(config=tf_config) as tf_sess:
      tf_sess.run(tf.global_variables_initializer())
      ckpt_state = tf.train.get_checkpoint_state(params["model_dir"])
    ...
    importer.import_graph_def(frozen_graph,
           {inputs.name: inputs},
           name="freeze")
    for i in logits_keys:
      logits[i] = tf.get_default_graph().get_tensor_by_name("model/freeze/" + logits[i].name)
