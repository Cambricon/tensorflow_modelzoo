...
def get_cpu_distribution_strategy(turn_off_distribution_strategy=False):
  """Return a DistributionStrategy for running the model on CPU.
     use_resource will be closed in this strategy.

  Args:
    turn_off_distribution_strategy: when set to True, do not use any
      distribution strategy. Otherwise, using CPU distribution strategy.
  ...
  if turn_off_distribution_strategy:
    ...
    return tf.contrib.distribute.CPUOneDeviceStrategy(var_strategy="CPU")

def get_mlu_distribution_strategy(distribution_strategy='one_device', num_mlus=0, all_reduce_alg=None):
  """Return a DistributionStrategy for running the model.

  Args:
    distribution_strategy: 'one_device', 'parameter_server'
    num_mlus: Number of MLUs to run this model.
  ...
  if distribution_strategy == 'one_device':
    if num_mlus == 0:
      ...
      return tf.contrib.distribute.OneDeviceStrategy("device:MLU:0")
    else:
      raise ValueError(('Not support mlu device numbers {} with one_device strategy.').format(num_mlus))
  if distribution_strategy == 'parameter_server':
    print('to be continued')
    #return tf.contrib.distribute.MLUParameterServerStrategy(num_mlus_per_worker=2)
