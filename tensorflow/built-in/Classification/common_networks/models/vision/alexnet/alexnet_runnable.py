...
# ==============================================================================
"""Runs a AlexNet model on the ImageNet dataset using custom training loops."""
...
from models.vision.alexnet import common
from models.vision.alexnet import imagenet_preprocessing
from models.vision.alexnet import alexnet_model


class AlexNetRunnable(standard_runnable.StandardTrainable,
                     standard_runnable.StandardEvaluable):
  """Implements the training and evaluation APIs for AlexNet model."""
  ...
  def __init__(self, flags_obj, time_callback, epoch_steps):
    ...
    self.model = alexnet_model.alexnet(
        num_classes=imagenet_preprocessing.NUM_CLASSES,
        batch_size=flags_obj.batch_size,
    ...
    def step_fn(inputs):
      ...
      with tf.GradientTape() as tape:
        ...
        if self.flags_obj.single_l2_loss_op:
          l2_loss = alexnet_model.L2_WEIGHT_DECAY * 2 * tf.add_n([
              tf.nn.l2_loss(v)
