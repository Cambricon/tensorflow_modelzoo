...
from models.modeling.hyperparams import base_config
from models.vision import augment
from models.vision import preprocessing
...
class DatasetBuilder:
  ...
  def load_records(self) -> tf.data.Dataset:
    ...
    if self.config.filenames is None:
      ...
      # Set seed due to hvd do this in every rank.
      # The result should be same, thus make sure every rank have different examples in a batch
      dataset = tf.data.Dataset.list_files(file_pattern, shuffle=True, seed=1)
    else:
  ...
  def load_synthetic(self) -> tf.data.Dataset:
    ...
    logging.info('===================== use_dummy_synthetic_data ===========================')
  ...
  def preprocess(self, image: tf.Tensor, label: tf.Tensor
    ...
    if self.is_training:
      ...
      image = preprocessing.preprocess_for_eval(
          image,
          image_size=self.image_size,
          num_channels=3, #self.num_channels,
          mean_subtract=self.config.mean_subtract,
          standardize=self.config.standardize,
          dtype=self.dtype)
