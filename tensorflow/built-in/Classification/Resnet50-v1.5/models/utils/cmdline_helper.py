...
def parse_cmdline(available_arch):
    ...
    p.add_argument('--batch_size', type=int, required=True, help="""Size of each minibatch per MLU.""")
    ...
    p.add_argument(
        '--data_format', choices=['NHWC', 'NCHW'], type=str, default='NHWC', required=False, help=argparse.SUPPRESS
    )
    ...
    _add_bool_argument(parser=p, name="use_hvd", default=False, required=False, help="If use horovod, please set True.")
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="""Specify ID of the target MLU on multi-device platform. Effective only for single-MLU mode."""
    )
