...
from mlu_data_utils import normalized_inputs
...
class ResnetModel(object):
    ...
    def __init__(
        ...
        if (not self.use_hvd_) or (self.use_hvd_ and hvd.rank() == 0):
            print("Model HParams:")
    ...
    def __call__(self, features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            mandatory_params = ["batch_size", "lr_init", "num_devices", "steps_per_epoch",
        ...
        if mode == tf.estimator.ModeKeys.TRAIN and not self.model_hparams.use_dali:
            ...
            with tf.device(device_string):
                ...
                device_prefetch_op, (features, labels) = self._stage([features, labels])
        ...
        with tf.device(device_string):
            ...
            if mode != tf.estimator.ModeKeys.PREDICT:
                ...
                if mixup != 0:
                    ...
                    reversed_feature_coefficients = tf.subtract(tf.ones(shape=feature_coefficients.shape),
                    ...
                    reversed_label_coefficients = tf.subtract(tf.ones(shape=label_coefficients.shape),
                                                              label_coefficients)
            ...
            if mode == tf.estimator.ModeKeys.TRAIN and params['quantize']:
                ...
                if params['symmetric']:
                    ...
                    tf.contrib.quantize.experimental_create_training_graph(tf.get_default_graph(), symmetric=True,
                                                                           use_qdq=params['use_qdq'],
                                                                           quant_delay=params['quant_delay'])
                else:
                    dllogger.log(data={"MODE": "USING ASSYMETRIC MODE"}, step=tuple())
                    tf.contrib.quantize.create_training_graph(tf.get_default_graph(), quant_delay=params['quant_delay'],
                                                              use_qdq=params['use_qdq'])
            ...
            if 'finetune_checkpoint' in params.keys():
                ...
                # manually restore global_step to ensure lr schedule is correct
                train_var_dict['global_step'] = global_step.op.name
                ...
                if mode == tf.estimator.ModeKeys.TRAIN:
                    ...
                    with tf.device("/cpu:0"):
                        ...
                        learning_rate = learning_rate_scheduler(
                            ...
                            # if hvd(n cards) finetune from ckpt(hvd n cards) or 1card finetune from ckpt(1card), num_batches_per_epoch=params["steps_per_epoch"],
                            # if hvd(n cards) finetune from ckpt(1 card), adjust num_batches_per_epoch to adjust lr
                            num_batches_per_epoch=params["steps_per_epoch"] if not params['hvd_finetune_test'] else
                            params["steps_per_epoch"] * hvd.size(),
                    ...
                    if self.model_hparams.use_dali:
                        ...
                        train_ops = tf.group(backprop_op, cpu_prefetch_op, device_prefetch_op, update_ops,
                                             name='train_ops')
