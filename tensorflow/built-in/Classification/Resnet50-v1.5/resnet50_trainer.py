...
from mlu_runner import Runner
from mlu_resnet import model_architectures
from cmdline_helper import parse_cmdline
...
if __name__ == "__main__":
    ...
    if FLAGS.use_horovod:
        ...
        global hvd
        hvd.init()
        devices = tf.config.experimental.list_physical_devices(FLAGS.device)
        if devices:
            tf.config.experimental.set_visible_devices(devices[hvd.local_rank()], FLAGS.device)
    ...
    runner = Runner(
        ...
        device_memory_fraction=FLAGS.device_memory_fraction,
        device=FLAGS.device,
        ...
        hvd_finetune_test=FLAGS.hvd_finetune_test,
    )
    ...
    if FLAGS.mode in ["train", "train_and_evaluate", "training_benchmark"]:
        runner.train(
            ...
            mixup=FLAGS.mixup,
            use_static_loss_scaling=FLAGS.use_amp,
    ...
    if FLAGS.mode in ["train_and_evaluate", 'evaluate', 'inference_benchmark']:
        ...
        if FLAGS.mode == 'inference_benchmark' and FLAGS.use_horovod:
            raise NotImplementedError("Only single MLU inference is implemented.")

        elif (not FLAGS.use_horovod) or (FLAGS.use_horovod and hvd.rank() == 0):
