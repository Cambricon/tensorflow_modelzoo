...
class Runner(object):
    ...
    def __init__(
        ...
        if use_gpu:
            ...
            os.environ['TF_MLU_THREAD_NODE'] = 'mlu_private'
        ...
        if self.use_performance and self.use_profiler:
    ...
    @staticmethod
    def _get_session_config(mode, xla, use_gpu):
        ...
        if hvd_utils.is_using_hvd():
            if use_gpu:
                ...
                config.mlu_options.visible_device_list = str(hvd.rank())
    ...
    @staticmethod
    def _get_run_config(mode, model_dir, xla, use_gpu, seed=None):
        ...
        config = tf.estimator.RunConfig(
            ...
            session_config=Runner._get_session_config(mode=mode, xla=xla, use_gpu=use_gpu),
    ...
    def _get_estimator(self, mode, run_params, xla, use_gpu):

        if mode not in ["train", 'validation', 'benchmark']:
            raise ValueError("Unknown mode received: %s (allowed: 'train', 'validation', 'benchmark')" % mode)

        run_config = Runner._get_run_config(
            mode=mode, model_dir=self.run_hparams.model_dir, xla=xla, use_gpu=use_gpu, seed=self.run_hparams.seed
        )
    ...
    def train(
        ...
        model = self._get_estimator(mode='train', run_params=estimator_params, xla=self.xla, use_gpu=self.use_gpu)
    ...
    def evaluate(self, iter_unit, num_iter, batch_size, warmup_steps=50, is_benchmark=False, save_eval_results_to_json=False):
        ...
        model = self._get_estimator(mode='validation', run_params=estimator_params, xla=self.xla, use_gpu=self.use_gpu)
