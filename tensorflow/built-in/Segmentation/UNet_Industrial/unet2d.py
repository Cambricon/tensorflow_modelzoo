...
if __name__ == "__main__":
    ...
    runner = Runner(
        ...
        use_gpu=RUNNING_CONFIG.use_gpu,
        use_performance=RUNNING_CONFIG.use_performance,
        use_profiler=RUNNING_CONFIG.use_profiler,
    ...
    if RUNNING_CONFIG.exec_mode in ["train_and_evaluate",'evaluate', 'inference_benchmark'] and hvd.rank() == 0:
        runner.evaluate(
            iter_unit=RUNNING_CONFIG.iter_unit if RUNNING_CONFIG.exec_mode != "evaluate" else "epoch",
            num_iter=RUNNING_CONFIG.num_iter if RUNNING_CONFIG.exec_mode != "evaluate" else 1,
            warmup_steps=RUNNING_CONFIG.warmup_steps,
            batch_size=RUNNING_CONFIG.batch_size,
