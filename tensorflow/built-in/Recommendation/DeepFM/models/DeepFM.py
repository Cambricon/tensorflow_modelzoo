...
class DeepFM(BaseEstimator, TransformerMixin):
    def __init__(self, feature_size, field_size,
        ...
        self.ckpt_file = params.model_dir + "/" + params.exec_mode
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            if params.ckpt_file:
                try:
                    self.saver.restore(self.sess, params.ckpt_file)
                    print("Restore weights from: %s ... " % params.ckpt_file)
                except:
                    print('%s does not exist !!!' % params.ckpt_file)
                    print('Now it starts to train network from scratch ...')
            if params.use_horovod:
    ...
    def fit(self, Xi_train, Xv_train, y_train,
        ...
        for epoch in range(self.epoch):
            ...
            if params.finetune_steps > 0:
                total_batch = params.finetune_steps
            ...
            if has_valid and (not params.use_horovod or (params.use_horovod and hvd.rank() == 0)):
                valid_result = self.evaluate(Xi_valid, Xv_valid, y_valid)
                self.valid_result.append(valid_result)
            if self.verbose > 0 and epoch % self.verbose == 0 and (not params.use_horovod or (params.use_horovod and hvd.rank() == 0)):
                if has_valid:
                    print("total_batch", total_batch, "[%d] train-result=%.4f, valid-result=%.4f [%.1f s]"
        ...
        if params.use_performance:
            if not params.use_horovod:
                write_json("summary", params.batch_size, times)
            elif (params.use_horovod and hvd.rank() == 0):
                write_json("summary", params.batch_size*hvd.size(), times)
