...
class ASRDataset(BaseDataset):
    ...
    def read_entries(self):
        ...
        if self.shuffle and not FLAGS.use_horovod:
    ...
    def preprocess(self, path: tf.Tensor, audio: tf.Tensor, indices: tf.Tensor,device):
    ...
    def tf_preprocess(self, path: tf.Tensor, audio: tf.Tensor, indices: tf.Tensor,device):
        with tf.device("/"+device+":0"):
    ...
    def parse(self, path: tf.Tensor, audio: tf.Tensor, indices: tf.Tensor):
        ...
        """
        mlus = tf.config.list_physical_devices("MLU")
        device = "CPU"
        if mlus:
            device = "MLU"
        data = self.tf_preprocess(path, audio, indices,device) if self.use_tf else self.preprocess(path, audio, indices,device)
    ...
    def process(self, dataset, batch_size):
        ...
        if FLAGS.use_horovod:
            ...
            self.total_steps = self.total_steps // hvd.size()
