...
def setup_devices(devices: List[int], gpu: bool = False):
    ...
    if gpu:
        ...
        if mlus:
            visible_mlus = [mlus[i] for i in devices]
            tf.config.set_visible_devices(visible_mlus, "MLU")
            logger.info(f"Run on {len(visible_mlus)} Physical MLUs")
...
def setup_strategy(devices: List[int], gpu: bool = False, tpu_address: str = None):
