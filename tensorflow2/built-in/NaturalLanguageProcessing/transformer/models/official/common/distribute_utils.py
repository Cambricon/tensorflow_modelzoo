...
def get_distribution_strategy(distribution_strategy="mirrored",
  ...
  if distribution_strategy == "off":
    ...
    if num_mlus > 1:
      raise ValueError(f"When {num_mlus} MLUs are specified, "
                       "distribution_strategy flag cannot be set to `off`.")
  ...
  if distribution_strategy == "one_device":
    if num_mlus == 0 and num_gpus == 0:
      return tf.distribute.OneDeviceStrategy("device:CPU:0")
    if num_mlus == 1 and num_gpus == 0:
      return tf.distribute.OneDeviceStrategy("device:MLU:0")
    if num_mlus == 0 and num_gpus == 1:
      return tf.distribute.OneDeviceStrategy("device:GPU:0")
    if num_mlus == 1 and num_gpus == 1:
      raise ValueError("`OneDeviceStrategy` can not be used for num_mlus == 1 and num_gpus == 1.")
    if (num_gpus > 1 or num_mlus > 1):
  ...
  if distribution_strategy == "mirrored":
    ...
    elif num_mlus > 0 and num_gpus <= 0:
      devices = ["device:MLU:%d" % i for i in range(num_mlus)]
    elif num_gpus > 0 and num_mlus <= 0:
      devices = ["device:GPU:%d" % i for i in range(num_gpus)]
    else:
      raise ValueError("Only one of num_mlus and num_gpus can be greater than 0")
