...
def eager_train_step(detection_model,
  ...
  tf.compat.v2.summary.image(
      ...
      max_outputs=3)
  if FLAGS.use_horovod and (tf.equal(global_step, tf.convert_to_tensor(0, dtype=tf.int64))):
...
def sweep_mlu_checkpoint(manager, mlu_checkpoint_name_queue):
  """Deletes or preserves managed mlu checkpoints."""
  from tensorflow.python.training.checkpoint_management import _delete_file_if_exists

  while len(mlu_checkpoint_name_queue) > manager._max_to_keep:
    filename = mlu_checkpoint_name_queue.popleft()
...
def train_loop(
  ...
  Args:
    ...
    batch_size_override: batch size for override batch size in pipeline_config.
    finetune_ckpt: checkpoint that used to finetune some steps to verify accuracy.
    use_profiler: Boolean, whether training and evaluation should run with profiler.
    use_performance: Boolean, whether using performance test tools for training.
  ...
  if batch_size_override:
    train_config.batch_size = batch_size_override
  ...
  summary_writer_filepath = get_filepath(strategy, log_dir)
  if record_summaries:
    ...
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        if train_config.fine_tune_checkpoint:
          tf.logging.info(
              'Load a fine-tuning checkpoint from {} with type of {}'.format(
        ...
        if finetune_ckpt is None:
          finetune_ckpt = tf.train.latest_checkpoint(model_dir)
        tf.logging.info(
            'Load checkpoint from "{}"'.format(finetune_ckpt))
        ckpt.restore(finetune_ckpt)
        ...
        tf.logging.info(
            'batch_size: {}, current_train_steps: {}, '
            'max_train_steps: {}, num_steps_per_iteration: {}'.format(
                train_config.batch_size,global_step.value(),
                train_steps, num_steps_per_iteration))
        ...
        for _ in range(global_step.value(), train_steps,
          ...
          if use_performance and ((FLAGS.use_horovod and hvd.rank() == 0) or (not FLAGS.use_horovod)):
          ...
          if ((int(global_step.value()) - checkpointed_step) >=
            ...
            if (FLAGS.use_horovod and hvd.rank() == 0) or (not FLAGS.use_horovod):
              save_path = manager.save()
        if use_performance and ((FLAGS.use_horovod and hvd.rank() == 0) or (not FLAGS.use_horovod)):
          global_batch_size = train_config.batch_size if not FLAGS.use_horovod else train_config.batch_size * hvd.size()
          write_json("summary", global_batch_size, time_record)
...
def eval_continuously(
    ...
    model_dir=None,
    checkpoint_dir=None,
    load_latest_ckpt_continuously=True,
    wait_interval=180,
    timeout=3600,
    eval_index=0,
    save_final_config=False,
  ...
  for latest_checkpoint in tf.train.checkpoints_iterator(
    ...
    if not load_latest_ckpt_continuously:
      break
