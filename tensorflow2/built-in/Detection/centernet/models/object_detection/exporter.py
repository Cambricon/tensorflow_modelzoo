...
def replace_variable_values_with_moving_averages(graph,
  ...
  with graph.as_default():
    ...
    with tf.Session() as sess:
      read_saver = tf.train.Saver(ema_variables_to_restore, max_to_keep=150)
      read_saver.restore(sess, current_checkpoint_file)
      write_saver = tf.train.Saver(max_to_keep=150)
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      saver = tf.train.Saver(
          saver_def=input_saver_def, save_relative_paths=True, max_to_keep=150)
...
def _export_inference_graph(input_type,
  ...
  saver = tf.train.Saver(**saver_kwargs, max_to_keep=150)
