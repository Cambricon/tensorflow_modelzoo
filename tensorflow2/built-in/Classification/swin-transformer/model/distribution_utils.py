...
def get_distribution_strategy(distribution_strategy="mirrored",
  ...
  """
  if (num_gpus < 0) or (num_mlus < 0):
    raise ValueError("`num_gpus`and`num_mlus`can not be negative.")
  ...
  if distribution_strategy == "off":
    ...
    if num_mlus > 1:
      raise ValueError(
          "When {} MLUs are specified, distribution_strategy "
          "flag cannot be set to 'off'.".format(num_mlus))
    return None

  if distribution_strategy == "tpu":
    # When tpu_address is an empty string, we communicate with local TPUs.
    raise ValueError(
      "TPU Distribution is not supported in Swin-transformer! Please "
      "use MLU or GPU to train and eval.")

  if distribution_strategy == "one_device":
    if (num_gpus == 0) and (num_mlus == 0):
      return tf.distribute.OneDeviceStrategy("device:CPU:0")
    elif (num_gpus > 1) or (num_mlus > 1):
      raise ValueError("`OneDeviceStrategy` can not be used for more than "
                       "one device.")
    elif (num_gpus == 1) and (num_mlus == 0):
      return tf.distribute.OneDeviceStrategy("device:GPU:0")
    elif (num_gpus == 0) and (num_mlus == 1):
      # When mlu has only one card, in order to avoid
      # modifying the graph, return None directly
      return None
    else:
      raise ValueError("Only one of mlu and gpu can be equal to 1")
  ...
  if distribution_strategy == "mirrored":
    if (num_gpus == 0) and (num_mlus == 0):
      ...
      distribution = tf.distribute.MirroredStrategy(devices=devices,
