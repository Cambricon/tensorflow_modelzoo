...
def SwinTransformer(model_name='swin_tiny_224', num_classes=1000, include_top=True, pretrained=True, 
                    use_tpu=False, cfgs=CFGS, pretrained_checkpoint=None, use_hvd=False,  horovod=None):
    cfg = cfgs[model_name]
    ...
    if not pretrained and (pretrained_checkpoint == "" or pretrained_checkpoint == None):
        return net
    pretrained_ckpt = os.path.join(os.getcwd(), model_name) if pretrained else pretrained_checkpoint
    if pretrained and (not use_hvd or (use_hvd and horovod.rank() == 0)):
        url = f'https://github.com/rishigami/Swin-Transformer-TF/releases/download/v0.1-tf-swin-weights/{model_name}.tgz'
        pretrained_ckpt = tf.keras.utils.get_file(pretrained_ckpt, url, untar=True)
        if not os.path.exists(pretrained_ckpt):
            os.system("tar -zxvf " + pretrained_ckpt + ".tar.gz")

    while pretrained and not os.path.exists(os.path.join(pretrained_ckpt, f'{model_name}.ckpt.index')):
        time.sleep(5)
