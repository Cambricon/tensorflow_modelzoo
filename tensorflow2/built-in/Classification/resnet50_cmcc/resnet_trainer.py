...
flags.DEFINE_integer(name='num_mlus', default=1,
                     help='The number of mlu devices.')

flags.DEFINE_boolean(name='use_horovod', default=False,
...
flags.DEFINE_boolean(name='use_performance', default=False,
                     help='Use performance.')

flags.DEFINE_boolean(name='use_profiler', default=False,
                     help='Use profiler.')

flags.DEFINE_boolean(name='use_amp', default=False,
                     help='Use amp.')

flags.DEFINE_integer('host_tracer_level', 0,
                     help='When using profiler for performance analysis, '
                          'you need to set host tracer level.')

flags.DEFINE_integer('device_tracer_level', 0,
                     help='When using profiler for performance analysis, '
                          'you need to set device tracer level.')

flags.DEFINE_string('profiler_dir', "./profiler",
                     help='When using profiler for performance analysis,'
...
def run(flags_obj):
  ...
  resnet_controller = orbit.Controller(
      ...
      summary_dir=flags_obj.model_dir if (flags_obj.use_horovod and hvd.rank() == 0) or not flags_obj.use_horovod else None,
  ...
  if flags_obj.mode == "train_and_eval":
    ...
    raise ValueError(
        'Must be set mode, `train_and_eval` or `train` or `eval`.')
  if (flags_obj.use_horovod and hvd.rank() == 0) or not flags_obj.use_horovod:
    time_callback.on_train_end()
  if flags_obj.use_performance:
    global_batch_size = flags_obj.batch_size if not flags_obj.use_horovod else flags_obj.batch_size * hvd.size()
    if (flags_obj.use_horovod and hvd.rank() == 0) or not flags_obj.use_horovod:
