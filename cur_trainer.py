...
class Trainer:
    ...
    def _wrap_model(self, model, training=True, dataloader=None):
        ...
        if self.args.n_gpu > 1 and not getattr(model, "is_loaded_in_8bit", False):
            rank=self.args.n_gpu
            world_size=self.args.n_gpu
            local_rank = self.args.local_rank
            print("andy: self.args:{}".format(self.args))
            print("andy:local_rank is:{},world_size:{},n_gpu:{}".format(local_rank,self.args.world_size,self.args.n_gpu))
            dist.init_process_group(backend='cncl', init_method=INIT_METHOD,
                                    rank=local_rank, world_size=world_size)
            model = DDP(model,device_ids = [local_rank])
            print("andy model dev:{}".format(model.device))
